...
check_SV <- function(state, cfg) {
  domains <- state$domains
  active_rules  <- state$active_rules

  if (is.null(domains$sv) || nrow(domains$sv) == 0) {
    message("  WARNING [SV]: domains$sv is empty - skipping all SV trial checks.")
    return(state)
  }
  ...
  # SVCHK001 - Missing visit start date (SVSTDTC)
  # ============================================================================
  if (isTRUE(active_rules["SVCHK001"])) {
    ...
    state <- collect_findings(state, SVCHK001, id = "SVCHK001")
  }

  # ============================================================================
  # SVCHK002 - Visit end date (SVENDTC) before visit start date (SVSTDTC)
  # ============================================================================
  if (isTRUE(active_rules["SVCHK002"])) {
    ...
    state <- collect_findings(state, SVCHK002, id = "SVCHK002")
  }

  # ============================================================================
  # SVCHK003 - Visit dates not in chronological order within a subject
  # ============================================================================
  if (isTRUE(active_rules["SVCHK003"])) {
    ...
    state <- collect_findings(state, SVCHK003, id = "SVCHK003")
  }
