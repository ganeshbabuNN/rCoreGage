# ==============================================================================
# SV_study.R  -  Subject Visits  -  Project level checks
#
...
#
# Input    : state$domains$sv  - loaded from SV.csv
# Rule_Set  : SV_PRJ
#
# Note     : Update study_end_date below to match your trial's planned end date
# ==============================================================================

check_SV_study <- function(state, cfg) {
  ...
  if (is.null(domains$sv) || nrow(domains$sv) == 0) {
    message("  WARNING [SV_study]: domains$sv is empty - skipping all SV project checks.")
    return(state)
  }
  ...
  # ============================================================================
  if (isTRUE(active_rules["SVPRJ001"])) {
    ...
    if (!"SVUPDES" %in% names(sv)) {
      ...
      state <- collect_findings(state, SVPRJ001, id = "SVPRJ001")
    }
  }
  ...
  # ============================================================================
  if (isTRUE(active_rules["SVPRJ002"])) {
    ...
    state <- collect_findings(state, SVPRJ002, id = "SVPRJ002")
  }

  # ============================================================================
  # SVPRJ003 - Visit date outside expected study window
  # ============================================================================
  if (isTRUE(active_rules["SVPRJ003"])) {
    ...
    state <- collect_findings(state, SVPRJ003, id = "SVPRJ003")
  }
