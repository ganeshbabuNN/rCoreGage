...
run_DM <- function(ctx, config) {
  data_list <- ctx$data_list
  switches  <- ctx$switches

  if (is.null(data_list$dm) || nrow(data_list$dm) == 0) {
    message("  WARNING [DM]: data_list$dm is empty - skipping all DM trial checks.")
    return(ctx)
  }
  ...
  if (isTRUE(switches["DMCHK001"])) {
    ...
    ctx <- prepare(ctx, DMCHK001, id = "DMCHK001")
  }

  # ============================================================================
  # DMCHK002 - Age outside inclusion criteria (18 to 80 years)
  # ============================================================================
  if (isTRUE(switches["DMCHK002"])) {
    ...
    ctx <- prepare(ctx, DMCHK002, id = "DMCHK002")
  }

  # ============================================================================
  # DMCHK003 - Missing reference start date (RFSTDTC)
  # ============================================================================
  if (isTRUE(switches["DMCHK003"])) {
    ...
    ctx <- prepare(ctx, DMCHK003, id = "DMCHK003")
  }
