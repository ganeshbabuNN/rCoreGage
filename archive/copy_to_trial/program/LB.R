...
# Batch_ID : LB
# ==============================================================================
run_LB <- function(ctx, config) {
  data_list <- ctx$data_list ; switches <- ctx$switches
  if (is.null(data_list$lb) || nrow(data_list$lb)==0) {
    message("  WARNING [LB]: data_list$lb is empty - skipping.") ; return(ctx)
  }
  lb <- data_list$lb

  # LBCHK001 : Lab value outside normal reference range
  if (isTRUE(switches["LBCHK001"])) {
    ...
    ctx <- prepare(ctx, LBCHK001, id="LBCHK001")
  }
  
  # LBCHK002 : Missing lab result value (LBORRES)
  if (isTRUE(switches["LBCHK002"])) {
    ...
    LBCHK002 <- lb |>
      ...
      dplyr::select(subj_id, vis_id, description)
    ctx <- prepare(ctx, LBCHK002, id="LBCHK002")
  }
  
  # LBCHK003 : Duplicate records for same subject/visit/test
  if (isTRUE(switches["LBCHK003"])) {
    ...
    LBCHK003 <- lb |>
      ...
      dplyr::select(subj_id, vis_id, description)
    ctx <- prepare(ctx, LBCHK003, id="LBCHK003")
  }
  ctx
}
