...
# Batch_ID : EX
# ==============================================================================
run_EX <- function(ctx, config) {
  data_list <- ctx$data_list ; switches <- ctx$switches
  if (is.null(data_list$ex) || nrow(data_list$ex)==0) {
    message("  WARNING [EX]: data_list$ex is empty - skipping.") ; return(ctx) }
  ex <- data_list$ex

  if (isTRUE(switches["EXCHK001"])) {
    ...
    ctx <- prepare(ctx,EXCHK001,id="EXCHK001") }

  if (isTRUE(switches["EXCHK002"])) {
    ...
    EXCHK002 <- ex |>
      ...
      dplyr::select(subj_id,vis_id,description)
    ctx <- prepare(ctx,EXCHK002,id="EXCHK002") }

  if (isTRUE(switches["EXCHK003"])) {
    ...
    EXCHK003 <- ex |>
      ...
      dplyr::select(subj_id,vis_id,description)
    ctx <- prepare(ctx,EXCHK003,id="EXCHK003") }
  ctx
}
